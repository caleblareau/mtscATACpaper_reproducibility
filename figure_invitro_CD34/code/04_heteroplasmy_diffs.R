...
mm <- fread("mitomap_cfrm_pathogenic.tsv", header = FALSE)
...
se500 <- readRDS("misc_mito/filtered_SE_CD34-500.rds")
se800 <- readRDS("misc_mito/filtered_SE_CD34-800.rds")
...
raw_500 <- data.frame(
  ...
  t(assays(se500)[["allele_frequency"]])
) %>% group_by(lib) %>%
...
raw_800 <- data.frame(
  ...
  t(assays(se800)[["allele_frequency"]])
) %>% group_by(lib) %>%
  summarize_at(vars(-group_cols()), list(mean)) %>% t()
df_800 <- data.frame(
...
cowplot::ggsave(cowplot::plot_grid(p1, p2, p3, p4, nrow = 1), 
...
pS <- ggplot(data.frame(l5, l8), aes(x = l5, y = l8)) +
  geom_point(size = 0.2) +
...
cowplot::ggsave(pS, file = "plots/plot_supp_18muts.pdf", width = 1.7, height = 1.7)
