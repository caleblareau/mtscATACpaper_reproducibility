...
dd <- readRDS("misc_mito/cluster-id-800.rds")
...
mdf %>% group_by(cluster,Group, timepoint) %>% summarize(count = n()) %>% data.frame() %>% head(1000) %>% tail(250)

mdf %>% group_by(cluster,Group, timepoint) %>% summarize(count = n()) %>% data.frame() %>%
  dplyr::filter(cluster %in% c("2", "14", "16", "53"))

mdf %>% group_by(cluster,Group, timepoint) %>% summarize(count = n()) %>% data.frame() %>% 
  dplyr::filter(Group == "ery6") %>% arrange(desc(count))
...
plot_d$bi1 <- plot_d$barcode  %in% pull_barcodes_dd(1, 13)
plot_d$bi2 <- plot_d$barcode  %in% pull_barcodes_dd(2, 13)
plot_d$bi3 <- plot_d$barcode  %in% pull_barcodes_dd(3,13)
...
plot_d$my1 <- plot_d$barcode  %in% pull_barcodes_dd(1, 30)
plot_d$my2 <- plot_d$barcode  %in% pull_barcodes_dd(2, 30)
plot_d$my3 <- plot_d$barcode  %in% pull_barcodes_dd(3,30)
...
#cowplot::ggsave(cowplot::plot_grid(pe12, pm12, pb12, pe3, pm3, pb3, nrow = 2),
...
round(Matrix::rowMeans(afin[,as.character(plot_d %>% dplyr::filter(ery3) %>% pull(barcode))]) %>% sort(), 3)
round(Matrix::rowMeans(afin[,as.character(plot_d %>% dplyr::filter(my3) %>% pull(barcode))]) %>% sort(), 3)
round(Matrix::rowMeans(afin[,as.character(plot_d %>% dplyr::filter(bi3) %>% pull(barcode))]) %>% sort(), 3)
...
p1 <- ggplot(observed, aes(x = rankSort, y = diff, fill = diff)) +
...
#cowplot::ggsave(p1, file = "plots/zscore_clones_bars.pdf", height = 1.5, width = 2.5)
...
dev <- assays(readRDS("chromVAR/CD34_800_tf_deviations.rds"))[["z"]]
colnames(dev) <- paste0("CD34_800-", as.character(colnames(dev)))
dev_df <- data.frame(
  rowData(readRDS("chromVAR/CD34_800_tf_deviations.rds")),
...
cowplot::ggsave(p2, file = "plots/zscore_TFs_dots_progenitors.pdf", height = 1.5, width = 2.5)
